...
function TabelaEditavel() {
  const [dados, setDados] = useState([
    { id: 1, curso: 'Curso 1', periodo: 'Período 1', editavel: false, cursoOriginal: '', periodoOriginal: '' },
    { id: 2, curso: 'Curso 2', periodo: 'Período 2', editavel: false, cursoOriginal: '', periodoOriginal: '' },
    { id: 3, curso: 'Curso 3', periodo: 'Período 3', editavel: false, cursoOriginal: '', periodoOriginal: '' },
  ]);

  useEffect(() => {
    const dadosArmazenados = localStorage.getItem('Storage_curso');
    if (dadosArmazenados) {
  ...
  const handleEdit = (id) => {
    const novosDados = dados.map(item => {
      if (item.id === id) {
        return { ...item, editavel: true, cursoOriginal: item.curso, periodoOriginal: item.periodo };
      }
      return item;
    });
    setDados(novosDados);
  ...
  const handleCancel = (id) => {
    const novosDados = dados.map(item => {
      if (item.id === id) {
        return { ...item, editavel: false, curso: item.cursoOriginal, periodo: item.periodoOriginal };
      }
      return item;
    });
    setDados(novosDados);
  ...
  const salvarDadosNoStorage = (dados) => {
    localStorage.setItem('Storage_curso', JSON.stringify(dados));
  };
  ...
  const excluirDoStorage = (id) => {
    ...
    if (dadosArmazenados) {
      ...
      localStorage.setItem('Storage_curso', JSON.stringify(dadosAtualizados));
    }
  };
